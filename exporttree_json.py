...
#Prompt:Directory
def loading_files():
    if os.path.isfile(r'.\pickle\json.pickle') is True and os.path.isfile(r'.\pickle\locfiles.pickle') is True:
        prev=True
    else:
        prev=False
    if prev is True:
        print('Would you like to use your previous JSON file? y/n:')
        usepre=input() 
    else:
        usepre=None
    if usepre=='y':
...
def variables():
    ...
    return faithde,faithre,cult,live,dead,other,dyn,title,houseid,skillist,start
...
def getperson():
    ...
    # LIVE Skills AND TRAITS
            ...
            if 'traits' in live[i]:
                ...
                traitcon[i.zfill(maxlen)]=traitxml
                if sex=='Female':
            ...
            if 'recessive_traits' in live[i]:
                ...
                traitcon[i.zfill(maxlen)].extend(traitxml2)
                if sex=='Female':
    ...
    # OTHER COMBINE
            ...
            allist.append(combine)
            print(f'{setlist.index(i)+1}/{len(setlist)} CHARACTERS PROCESSED')
    traitcon={k: v for k, v in traitcon.items() if v}
    with open(r'.\pickle\traitpic.pickle','wb') as f:
        pickle.dump(traitcon,f)
    print('Created pickle binary storing idnum-trait relationship')
    with open(r'.\pickle\faithpic.pickle','wb') as f:
        pickle.dump(faithcon,f)
    print('Created pickle binary storing idnum-faith relationship')
...
#get traits/faith-idnum relationship
    with open(r'.\pickle\traitpic.pickle','rb') as f:
        traitpic=pickle.load(f)
    with open(r'.\pickle\faithpic.pickle','rb') as f:
...
#convert dds icons to png and save to main dir 
    while True:
        if os.path.isdir('faith_icon') and os.path.isdir('traits_icon')  is True:
            print('Using previously converted Faith and Traits icons.')
            break
        else:
            print(r'Enter your ck3 game directory (eg. C:\Program Files (x86)\Steam\steamapps\common\Crusader Kings III\game):')
            yourinput=input()
...
[3] Automatically Import Tree into Gramps with Traits/Faith Icons (Must use a CSV file as a base. Gramps program MUST be closed.)
[4] Help
[5] Exit Program""")
    choice=input()
    if choice=='4':
...
[3] - Automatically import your save into Gramps with corresponding faith and traits icons. The icons in your game directory will be converted to png format and placed in the directory of this program. 
This operation will convert your csv file into gramps-xml, edit the xml to link the icons to each of your characters, and import your save into Gramps.
    ...
    if choice=='2':
        try:
            ...
            faithde,faithre,cult,live,dead,other,dyn,title,houseid,skillist,start=variables()
