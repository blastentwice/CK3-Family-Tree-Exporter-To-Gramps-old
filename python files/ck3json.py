...
exe_path = os.path.join(base_path, 'binaries\ck3json.exe')
exe_path2=os.path.join(base_path, 'binaries\DDStronk.exe')
exe_path3=os.path.join(base_path, 'traits\\traits.csv')
exe_path4=os.path.join(base_path, 'traits\\ftraits.csv')
exe_path5=os.path.join(base_path, 'traits\\traitcode.csv')
# In[ ]:
...
def loadall(jsondir,gamedir):
    try:
        with open(jsondir,encoding='utf-8') as f:
            data=json.load(f)
        with open (fr'{gamedir}/localization/english/names/character_names_l_english.yml',encoding='utf-8') as f:
            locnames=f.read()
        ...
        with open(exe_path3,encoding='utf-8') as f:
        ...
        with open(exe_path4,encoding='utf-8') as f:
        ...
        with open(exe_path5,encoding='utf-8') as f:
...
def keepog():
    ...
    if og==False:
        og=True
        with open(r'./pickle/ogname.pickle','wb') as f:
            pickle.dump(og,f)
        print('Saved selection as ogname.pickle')
...
#Convert to JSON using ck3json program
    try:
        if bool(ck3n) is True:
            ...
            p1=(subprocess.run(fr'"{exe_path}" "{gamestatenew}"',shell=True,capture_output=True))
            raw=p1.stdout.decode()

        else:
            print('File is being converted to JSON...')
            p1=(subprocess.run(fr'"{exe_path}" "{ironman}" ck3bin',shell=True,capture_output=True))
            raw=p1.stdout.decode()
...
#convert dds icons to png and save to main dir 
    while True:
        if os.path.isdir('icon/faith_icon') and os.path.isdir('icon/traits_icon')  is True and os.path.isdir('icon/coa_icon'):
    ...
    if os.path.isdir('icon/faith_icon') is False and os.path.isdir('icon/traits_icon') is False and os.path.isdir('coa_template') is False:
        os.makedirs('icon/faith_icon',exist_ok=True)
        os.makedirs('icon/traits_icon',exist_ok=True)
        os.makedirs('icon/coa_icon',exist_ok=True)
        os.makedirs('icon/coa_icon/rank',exist_ok=True)
        os.makedirs('icon/coa_icon/colored_emblems',exist_ok=True)
        os.makedirs('icon/coa_icon/patterns',exist_ok=True)
        os.makedirs('icon/coa_icon/exported_coa',exist_ok=True)
        
        path=fr'{yourinput}/gfx/interface/icons/faith'
        path2=fr'{yourinput}/gfx/interface/icons/traits'
        ...
        pathpng=fr'{path}\*png'
        pathpng2=fr'{path2}\*png'
        pathpng3=fr'{path3}\*png'
        pathpng4=fr'{path4}\*png'
        pathpng5=fr'{path5}\*png'

        p1=(subprocess.run(fr'"{exe_path2}" "{path}" && move "{pathpng}" icon/faith_icon',shell=True,capture_output=True,input=b'yes')) 
        error1=p1.stderr.decode()
        print(error1)
        p1=(subprocess.run(fr'"{exe_path2}" "{path2}" && move "{pathpng2}" icon/traits_icon',shell=True,capture_output=True,input=b'yes'))
        p1=(subprocess.run(fr'"{exe_path2}" "{path3}" && move "{pathpng3}" icon/coa_icon/rank',shell=True,capture_output=True,input=b'yes'))
        p1=(subprocess.run(fr'"{exe_path2}" "{path4}" && move "{pathpng4}" icon/coa_icon/colored_emblems',shell=True,capture_output=True,input=b'yes'))
        p1=(subprocess.run(fr'"{exe_path2}" "{path5}" && move "{pathpng5}" icon/coa_icon/patterns',shell=True,capture_output=True,input=b'yes'))
...
#Producing Coa files
    ...
    os.makedirs(f'icon/coa_icon/exported_coa/{outputfilename}',exist_ok=True)
    color.maincoaex(f'icon/coa_icon/exported_coa/{outputfilename}')
    print('Exported title and dynasty icons saved to icon/coa_icon folder')
    
#Parse XML and add obj element
    iconlist=os.listdir('icon/faith_icon')
    traitlist=os.listdir('icon/traits_icon')
    sexlist=os.listdir('icon/avatar')
    coalist=os.listdir(f'icon/coa_icon/exported_coa/{outputfilename}')
    iconrel=['icon/faith_icon/'+i for i in iconlist]
    traitrel=['icon/traits_icon/'+i for i in traitlist]
    sexrel=['icon/avatar/'+i for i in sexlist]
    coarel=[f'icon/coa_icon/exported_coa/{outputfilename}/'+i for i in coalist]
...
if os.path.isfile(r'./pickle/ogname.pickle') is True:
    with open(r'./pickle/ogname.pickle','rb') as f:
        og=pickle.load(f)
else:
    og=False
    os.makedirs('pickle',exist_ok=True)
...
while True:
    print(fr"""Main Menu
Select Operations:
[0] Preserve Original Spelling Of Name (Currently {og})
...
[4] - Same as [3] except Dynasty and Title icons are excluded. This mode will use the gender icons in the avatar folder as thumbnails for each character. Performance may be better
and should be selected if [3] lags too much.
    ...
    if choice=='2':
        try:
            ...
            if og is False:
                print('Use Anglicized Names')
                allist=getperson()
            elif og is True:
                print('Use Original Names')
                allist=getpersonOG()
            end=maketable()
            timer(start,end)
        except Exception as e:
